...
template <unsigned SIZE> class CKMC {
	...
	template<typename COUNTER_TYPE>	bool ProcessSmallKOptimization(const CHashModuloFilter& moduloFilter);
...
bool CKMC<SIZE>::ProcessSmallKOptimization(const CHashModuloFilter& moduloFilter)
{	
	...
	{
		w_small_k_splitters[i] = new CWSmallKSplitter<COUNTER_TYPE>(Params, Queues, moduloFilter);
...
template <unsigned SIZE> bool CKMC<SIZE>::Process()
{
	...
	CHashModuloFilter moduloFilter(Params.modulo);
	...
	if (AdjustMemoryLimitsSmallK())
	{
		...
		if ((uint64)Params.cutoff_max > ((1ull << 32) - 1))
			return ProcessSmallKOptimization<uint64>(moduloFilter);
		else
			return ProcessSmallKOptimization<uint32>(moduloFilter);
	}
	...
	for (int i = 0; i < Params.n_splitters; ++i)
	{
		w_stats_splitters[i] = new CWStatsSplitter(Params, Queues, moduloFilter);
	...
	for(int i = 0; i < Params.n_splitters; ++i)
	{
		w_splitters[i] = new CWSplitter(Params, Queues, moduloFilter);
...

	{
		...
		{
			w_sorters[i] = new CWKmerBinSorter<SIZE>(Params, Queues, sort_func, moduloFilter);
