...
private:
	const CHashModuloFilter& moduloFilter;
...
public:
	...
	CKmerBinSorter(CKMCParams &Params, CKMCQueues &Queues, SortFunction<CKmer<SIZE>> sort_func, const CHashModuloFilter& moduloFilter);
	~CKmerBinSorter();
...
template <unsigned SIZE> CKmerBinSorter<SIZE>::CKmerBinSorter(CKMCParams &Params, CKMCQueues &Queues, SortFunction<CKmer<SIZE>> sort_func, const CHashModuloFilter& moduloFilter)
:
...
template <unsigned SIZE> void CKmerBinSorter<SIZE>::ExpandKmersBoth(uint64 tmp_size)
{
	...
	while (pos < tmp_size)
	{
		...
		if(moduloFilter.checkModuloHash(kmer_can.get_lowest()))
...
public:	
	CWKmerBinSorter(CKMCParams &Params, CKMCQueues &Queues, SortFunction<CKmer<SIZE>> sort_func, const CHashModuloFilter& moduloFilter);
	~CWKmerBinSorter();
	void GetDebugStats(uint64& _sum_n_recs, uint64& _sum_n_plus_x_recs)
	{
...
//----------------------------------------------------------------------------------
// Constructor
template <unsigned SIZE> CWKmerBinSorter<SIZE>::CWKmerBinSorter(CKMCParams &Params, CKMCQueues &Queues, SortFunction<CKmer<SIZE>> sort_func, const CHashModuloFilter& moduloFilter)
{
	kbs = new CKmerBinSorter<SIZE>(Params, Queues, sort_func, moduloFilter);
}
