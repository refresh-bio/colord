...
//		else 
	...
	if (Params.modulo == 0)
	{
		std::cerr << "Error: -f <modulo> must be specified\n";
		return false;
	}
	return true;
}
...
int run_filtering_kmc(const CFilteringParams& params)
{
	...
	auto regOpt = [&opts](const std::string& opt)
	{
		opts.push_back(opt);
	};

	auto optsToArgvs = [&opts, &argv]
	{
	...
	regOpt("-k" + std::to_string(params.kmerLen));
	...
	regOpt("-f" + std::to_string(params.modulo));

	regOpt("-j" + params.statsFile);
	...
	regOpt(params.outputPath);

	regOpt(params.tmpPath);

	optsToArgvs();

	return old_main(argv.size(), argv.data());
}
