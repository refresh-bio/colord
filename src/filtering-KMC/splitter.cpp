...
//----------------------------------------------------------------------------------
// Assigns queues
CSplitter::CSplitter(CKMCParams &Params, CKMCQueues &Queues, const CHashModuloFilter& moduloFilter):
...
void CSplitter::InitBins(CKMCParams &Params, CKMCQueues &Queues)
{
	...
	{
		bins[i] = new CKmerBinCollector(Queues, Params, buffer_size, i, moduloFilter);
	...
	if (both_strands)
		while (GetSeq(seq, seq_size, read_type))
		{
			...
			for (; i < seq_size; ++i)
			{
				...
				if (moduloFilter.checkModuloHash(kmer_can.data))
				{
...
//----------------------------------------------------------------------------------
// Constructor
CWSplitter::CWSplitter(CKMCParams &Params, CKMCQueues &Queues, const CHashModuloFilter& moduloFilter)
{	
	pq = Queues.part_queue;
	bpq = Queues.bpq;
	pmm_fastq = Queues.pmm_fastq;
	spl = new CSplitter(Params, Queues, moduloFilter);
...
//----------------------------------------------------------------------------------
// Constructor
CWStatsSplitter::CWStatsSplitter(CKMCParams &Params, CKMCQueues &Queues, const CHashModuloFilter& moduloFilter)
{
...
//----------------------------------------------------------------------------------
// Constructor
template <typename COUNTER_TYPE> CWSmallKSplitter<COUNTER_TYPE>::CWSmallKSplitter(CKMCParams &Params, CKMCQueues &Queues, const CHashModuloFilter& moduloFilter)
{
	pq = Queues.part_queue;
	pmm_fastq = Queues.pmm_fastq;
